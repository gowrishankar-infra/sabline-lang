...
def key_fingerprint(key: bytes) -> str:
    ...
    import hashlib
    return hashlib.sha256(b"sabline key fingerprint\0" + key).hexdigest()[:12]
...
def run_digest(name: str, args: list[Any], line: int) -> Any:
    ...
    if not messages:
        raise SablineError("E609", f"'{name}' was given nothing to sign", line,
...
def run_money(name: str, args: list[Any], line: int) -> Any:
    ...
    if name == "percent_of":
        ...
        if den == 0:
            raise SablineError("E403", "percent_of with a denominator of "
                               "zero", line,
...
def run_builtin(name: str, args: list[Any], line: int) -> Any:
    ...
    if name == "round":
        ...
        if x != x or x in (float("inf"), float("-inf")) \
            ...
            raise SablineError("E407", f"round({x!r}) has no whole number "
    ...
    if name == "split":
        if args[1] == "":
            raise SablineError("E609", "cannot split by empty text", line,
    ...
    if name == "request":
        ...
        headers.setdefault("User-Agent", f"sabline/{VERSION}")
    ...
    if name in ("fetch", "post", "fetch_status"):
        ...
        if not (url.startswith("http://") or url.startswith("https://")):
            url = "https://" + url
        headers = {"User-Agent": f"sabline/{VERSION}"}
        data = None
        if name == "post":
            body = str(args[1])
    ...
    if name == "random":
        n = args[0]
        if n <= 0:
            raise SablineError("E405", "random(n) needs n greater than 0", line,
...
def build_runtime(funcs: list[Function], native: dict[Any, Any] | None = None) -> dict[str, Any]:
    ...
    # a stop asked for from outside (8.3, sabline eval): the file the parent
    ...
    # each Sabline frame costs several Python frames, so lift Python's
    ...
    def eval_(node: Any, env: Any) -> Any:
        ...
        if cls is Var:                  # the commonest node by far
            ...
            if name in table:
                return table[name]
            raise SablineError("E402", f"unknown variable '{name}'",
                               node.line,
        ...
        if cls is Var:
            ...
            raise SablineError("E402", f"unknown variable '{node.name}'", node.line,
        ...
        if cls is BinOp:
            ...
            if node.op == "/":
                if r == 0:
                    raise SablineError("E403", "division by zero", node.line,
            ...
            if node.op == "%":
                if r == 0:
                    raise SablineError("E403", "remainder by zero", node.line,
...
def _needs_big_stack() -> bool:
    ...
    the raised recursion limit build_runtime sets, a deeply recursive Sabline
...
def _run_on_big_stack(fn: Callable[[], _R]) -> _R:
    ...
    re-raise whatever it raised (a SablineError, a FailSignal, or the
    ...
    def worker() -> None:
        ...
        try:
            box["value"] = fn()
        except BaseException as e:          # SablineError/FailSignal/SystemExit
            box["error"] = e
        finally:
            if on_main:
...
def interpret(funcs: list[Function], native: dict[Any, Any] | None = None) -> None:
    ...
    if "main" not in rt["table"]:
        raise SablineError("E400", "no 'main' function found", 1,
