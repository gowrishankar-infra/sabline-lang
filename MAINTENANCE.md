...
| crates.io | nothing stored | release.yml publishes the `sabline-rt` crate by OIDC trusted publishing, as PyPI, npm and the MCP registry already do. The trusted publisher on crates.io names this repository, `release.yml` and the environment `release`; RELEASING.md says how it was set up, and why the very first publish could not use it |
