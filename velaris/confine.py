...
fault in velaris itself is a crash inside a box and not an escape.
"""
import ctypes
import hashlib
import json
...
POLICY_SCHEMA = "velaris.os-policy/1"
FULL, PARTIAL, NONE = "full", "partial", "none"

# layer names, as receipts and `velaris doctor` write them
LANDLOCK = "landlock"
SECCOMP = "seccomp"
...
# not from Velaris, attempts one effect outside the budget. `read:PATH`,
...
FAULT_ENV = "VELARIS_FAULT_INJECT"
...
EXEMPT: tuple[tuple[str, str], ...] = (
    ("`velaris.run()` with no `timeout` and no `max_memory_mb`",
     "it runs in the caller's process, which Velaris must not confine: "
     "Landlock, seccomp, a sandbox profile and a lowered token cannot be "
     "taken off again. Pass a limit, or use a Pool."),
    ("the REPL, `velaris test`, `velaris bench`",
...
def supported() -> tuple[bool, str]:
    """Whether this system offers anything, and what, for `velaris doctor`."""
    if sys.platform.startswith("linux"):
...
def predict(policy: dict[str, Any], *, read_any: bool = False,
    ...
    `velaris doctor`, the command line's audit - it asks the kernel what it
...
def private_temp() -> tuple[str | None, str | None]:
    ...
    try:
        if sys.platform.startswith("linux"):
            ...
            made = rule = tempfile.mkdtemp(prefix="velaris-run-")
    except OSError:
        return None, None
...
def inject_fault(confinement: dict[str, Any] | None) -> None:
    """When VELARIS_FAULT_INJECT is set: the runtime itself, from Python,
    ...
    asked = os.environ.get(FAULT_ENV)
    if not asked:
        return
    ...
    if what is None:
        print(f"velaris: {FAULT_ENV}={asked!r} names no fault; it is read:"
    ...
    try:
        ...
        print(f"velaris: fault injection: {what} failed, and not by "
              f"confinement, which is none: {type(e).__name__}: {e}",
              file=sys.stderr)
        return
    print(f"velaris: fault injection: {what} succeeded; confinement: "
