...
| Runs that are not confined | An in-process `sabline.run()` with no `timeout` and no `max_memory_mb` runs in the caller's process, which Sabline must not confine; the REPL, `sabline test` and `sabline bench` run many programs in one process; and a budget that grants `ffi:os`, `ffi:subprocess`, plain `ffi` or a module the table does not name widens the OS policy to nothing enforced. Each says `"confinement": "none"` and why, and from 8.7 a command-line run whose grants turn the operating system layer off - any of these four, a module that does not exist included - also says so on stderr, once, as it starts. `sabline replay` does not print that line, though a replayed receipt whose grants turn the layer off is replayed with it off: replay runs the program inside a captured run and writes the program's stderr after the run has ended, so a line written from inside it would arrive late and among the program's own output, and the line was put only where `sabline FILE`, `run` and `trace` start a run. The replayed receipt says `none` and why, and replay compares its confinement level with the recorded one; printing the line before the run, from the receipt's budget, can be done and has not been. And a confined process is still a process of the user's: what the budget grants, it does with the user's own rights. *Closed by:* M4 in part, and only for a run that opts in: under `--confine strict` ([decisions/0006](../decisions/0006-where-sabline-loses.md) section e, accepted) an `ffi` grant widens nothing, and the OS policy is the budget's grants alone. Without it, and for the other runs in this row, nothing changes. | Give `run()` a timeout, or use a `Pool`, so the run has a process of its own; name `ffi` modules the table knows; run code you have not read as a different user, or in a container or a virtual machine, when the stakes warrant it. |
