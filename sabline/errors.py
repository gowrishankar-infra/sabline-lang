"""SablineError, and the table of every error code it can carry.
"""
import json
...
ERROR_TABLE = {
    "E000": "a character the lexer cannot read, or a run that stopped "
            "without a Sabline error to report",
    ...
    "E319": "the operating system's confinement refused an effect the "
            ...
            "under the fault-injection hook, SABLINE_FAULT_INJECT)",
    ...
    "E551": "a currency that is not in sabline.CURRENCIES, or one not "
    ...
    "E615": "a run under sabline eval was asked to stop from outside, and "
    ...
    "E999": "the self-test in 'sabline doctor' failed",
}
...
def _too_deep_error(running: bool) -> "SablineError":
...
# The name this class had until the project was renamed Sabline in 8.6.
# It is the same class, not a subclass, so `except VelarisError` catches
# what `except SablineError` catches and an `isinstance` check against
# either answers the same. Removed no sooner than 9.0 (STABILITY.md).
VelarisError = SablineError
