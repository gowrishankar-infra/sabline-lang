"""velaris permissions-ratchet: the permissions a repository's GitHub
Actions workflows give their jobs, at the working tree against a git ref.

    velaris permissions-ratchet --against REF [--json] [--root DIR]
...
Reading
-------
This reads YAML without a YAML library - Velaris's runtime has no
...
The JSON `--json` prints, velaris.permissions-ratchet/1, is provisional:
...
PERMISSIONS_RATCHET_SCHEMA = "velaris.permissions-ratchet/1"
...
def permissions_compare(
    ...
    result is the velaris.permissions-ratchet/1 document (provisional):
...
def permissions_lines(result: dict[str, Any]) -> list[str]:
    """What `velaris permissions-ratchet` prints: each widening, each
...
def permissions_main(argv: list[Any]) -> int:
    """velaris permissions-ratchet --against REF [--json] [--root DIR]"""
    usage = ("usage: velaris permissions-ratchet --against REF [--json] "
             "[--root DIR]")
    opts: dict[str, Any] = {}
    i = 0
    while i < len(argv):
    ...
    if not os.path.isdir(root):
        print(_ascii(f"velaris permissions-ratchet: {root} is not a "
                     f"directory"), file=sys.stderr)
        return 2
    try:
        result = permissions_ratchet(opts["--against"], root)
    except RuntimeError as e:
        print(_ascii(f"velaris permissions-ratchet: {e}"), file=sys.stderr)
        return 2
    if "--json" in opts:
