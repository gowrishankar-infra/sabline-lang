...
MCP_TOOLS_SCHEMA = "sabline.mcp-tools/1"
...
def mcp_tool_manifest(server_info: dict[Any, Any], tools: list[Any]) -> dict[str, Any]:
    """sabline.mcp-tools/1: every tool a server offers, hashed."""
    ...
    try:
        send({"jsonrpc": "2.0", "id": 1, "method": "initialize",
              "params": {"protocolVersion": "2024-11-05", "capabilities": {},
                         "clientInfo": {"name": "sabline mcp-verify",
                                        "version": VERSION}}})
        info = answer_to(1)
...
def _default_mcp_command() -> list[Any] | None:
    ...
    return [sys.executable, "-m", "sabline_mcp"]
...
def mcp_manifest_main(argv: list[Any]) -> int:
    """sabline mcp-manifest [-o FILE] [-- server command...]"""
    own, command = _split_server_command(argv)
    if not command:
        print("mcp-manifest: name the server to ask after --, as: sabline "
              "mcp-manifest -o tools.json -- python -m sabline_mcp",
              file=sys.stderr)
        return 2
    out = None
    if "-o" in own:
...
def mcp_verify_main(argv: list[Any]) -> int:
    """sabline mcp-verify MANIFEST [--bundle FILE] [--identity URL]
    ...
    while i < len(own):
        ...
        if a == "--skip-signature":
            ...
            print("usage: sabline mcp-verify MANIFEST [--bundle FILE] "
    ...
    if manifest_path is None:
        ...
        return 2
    if not command:
        print("mcp-verify: name the server to check after --, as: sabline "
              "mcp-verify tools.json -- python -m sabline_mcp",
              file=sys.stderr)
        return 2
    try:
    ...
    if not (isinstance(manifest, dict)
            and naming.schema_matches(manifest.get("schema"),
                                      MCP_TOOLS_SCHEMA)
