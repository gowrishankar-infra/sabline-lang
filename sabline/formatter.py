"""sabline fmt: one canonical style.
"""
import sys
...
def fmt_main(argv: list[str]) -> int:
    ...
    if not files:
        print("usage: sabline fmt <file.vel> [--stdout | --check]",
              file=sys.stderr)
        return 1
    status = 0
    for path in files:
        ...
        except (OSError, SablineError) as e:
            msg = e.human(path) if isinstance(e, SablineError) else str(e)
