"""sabline eval: one program run the way an evaluation sandbox runs code it
was handed (8.3).
"""
import json
import os
...
#
#     `sabline eval program.vel --receipt FILE` runs one program under a
...
EVAL_SCHEMA = "sabline.eval/1"                       # provisional
RECEIPT_STREAM_SCHEMA = "sabline.receipt-stream/1"   # provisional
...
# refused here; any other flag sabline eval does not know is refused too
EVAL_REFUSED_FLAGS = (
    ...
    while i < len(argv):
        ...
        if a in _SWITCHES:
            ...
            raise EvalRefused(f"{name}: {why}" if why else
                              f"{name} is not a flag of sabline eval")
        else:
            opts["files"].append(a)
        i += 1
    return opts
...
def eval_main(argv: list[str], program_words: list[str] | None = None) -> int:
    """sabline eval program.vel (--receipt FILE | --receipt-url URL) ..."""
    try:
        opts = _parse(list(argv))
    except EvalRefused as e:
        print(f"sabline eval: refused: {e}", file=sys.stderr)
        return 2
...
def _eval(opts: dict[str, Any], program_words: list[str]) -> int:
    ...
    import uuid
    if len(opts["files"]) != 1:
        raise EvalRefused("name one program: sabline eval program.vel "
                          "--receipt FILE")
    ...
    home = tempfile.mkdtemp(prefix="sabline-eval-")
    ...
    print(f"sabline eval: {predicate['exit']['outcome']}"
...
def confinement_probe(as_json: bool) -> int:
    """sabline eval --confinement-probe: start a worker confined as a run
    ...
    import shutil
    import socket
    home = tempfile.mkdtemp(prefix="sabline-eval-probe-")
    work = os.path.join(home, "tmp")
    os.makedirs(work)
    ...
    try:
        ...
        print(f"sabline eval: {e}", file=sys.stderr)
    finally:
        pool.close()
        listener.close()
        shutil.rmtree(home, ignore_errors=True)
    ...
    if not isinstance(tried, dict):
        print(f"sabline eval: the confined worker gave no answer "
