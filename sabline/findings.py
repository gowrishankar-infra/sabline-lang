...
#     The verifier lives here and not in sabline_mcp.py, so a changed
...
SARIF_FINDINGS = (
    ...
    ("capability-widened", "error",
     "code that needs a grant sabline.capabilities does not give: an "
    ...
    ("capability-effect-gained", "error",
     "a function sabline.capabilities records now declares an effect it "
     "did not declare there (capabilities check)"),
    ("capability-narrowed", "note",
     "sabline.capabilities gives more than the code needs now; "
     "capabilities init --force records the narrower surface"),
    ("dependency-capability-widened", "error",
     "an upgraded Sabline dependency needs a grant its previous version "
    ...
    ("dependency-effect-gained", "error",
     "a function of an upgraded Sabline dependency declares an effect it "
    ...
    ("dependency-surface-unknown", "note",
     "an upgraded dependency holds code other than Sabline, or could not "
    ...
    ("dependency-narrowed", "note",
     "an upgraded Sabline dependency needs less than its previous version "
     "did (deps-diff)"),
)


def sarif_rules() -> list[Any]:
    """Every rule a Sabline SARIF log can cite: one per entry of the
...
class _SarifRun:
    ...
    What is deliberately not written: a Sabline fix is a sentence, and
    ...
    def log(self) -> dict[str, Any]:
        ...
        return {"$schema": SARIF_SCHEMA_URI, "version": "2.1.0",
                "runs": [{
                    "tool": {"driver": {
                        "name": "Sabline", "version": VERSION,
                        "semanticVersion": VERSION,
                        "informationUri": REPOSITORY,
                        "rules": self.rules}},
                    "automationDetails": {"id": f"sabline/{self.command}/"},
...
def sarif_check(targets: list[Any], strict: bool = False) -> tuple[dict[Any, Any], int]:
    """`sabline check --sarif`: (the SARIF log, the exit code the plain
...
def sarif_proofs(reports: dict[Any, Any], totals: dict[Any, Any], share: float,
    ...
    """`sabline proofs --sarif`: what did not compile, and every promise
...
def sarif_audit(files: list[Any], strict: bool = False,
    ...
    """`sabline audit --sarif`: the audit of each file as findings - what
    ...
    secret-declassified) - and the sabline.audit/1 document of each file,
...
INVOCATION_SCHEMA = "sabline.invocation/1"
