"""sabline receipts diff: what a run did that its program's audit does not
...
#
#     against an audit - of the program, or a sabline.audit/1 document, or a
...
DIFF_SCHEMA = "sabline.receipts-diff/1"      # provisional (STABILITY.md)
...
def load_audit(path: str) -> tuple[dict[str, Any], list[Any] | None, str]:
    ...
    program is attested here; a file is a sabline.audit/1 document or a
    ...
    if isinstance(document, dict) and naming.schema_matches(
            document.get("schema"), "sabline.audit/1"):
    ...
    if isinstance(document, dict) and "_type" in document:
        ...
        raise Unread(f"{path} is not a capability Statement this reads: "
                     + "; ".join(problems or ["another predicate type"]))
    raise Unread(f"{path} is not a sabline.audit/1 document, a capability "
...
def diff_report(receipt_path: str, audit_path: str | None,
    ...
    """The sabline.receipts-diff/1 report, with its exit status in 'exit'."""
...
def receipts_main(argv: list[Any]) -> int:
    """sabline receipts diff RECEIPT [--audit PROGRAM|AUDIT|STATEMENT]
    [--against RECEIPT|DIR ...] [--json]"""
    usage = ("usage: sabline receipts diff <receipt> [--audit PROGRAM | "
    ...
    while i < len(argv):
        ...
        if a in ("--audit", "--against"):
            if i + 1 >= len(argv):
                print(f"sabline receipts diff: {a} needs a value",
                      file=sys.stderr)
                return 2
            if a == "--audit":
                if audit is not None:
                    print("sabline receipts diff: --audit is given twice",
                          file=sys.stderr)
                    return 2
                audit = argv[i + 1]
            else:
