"""sabline.Pool: bounded runs without a new interpreter each time, and nothing
...
#     which costs about a tenth of a second before a line of Sabline is
...
def pool_worker(argv: list[Any]) -> int:
    """One long-lived child behind sabline.Pool.
    ...
    # sabline eval's worker (8.3): where a stop is asked for, and the
    ...
    try:
        ...
        sys.stderr.write(f"sabline worker: {e}\n")
        return 2
...
class _Worker:
    ...
    def __init__(self, cmd: list[Any], max_memory_mb: Any,
        ...
        # and refuses any further process (8.3, sabline eval).
...
class Pool:
    """Long-lived workers for bounded runs, all under ONE budget.
        ...
        pool = sabline.Pool(size=4, allow={"io"}, timeout=30,
                            max_memory_mb=512)
    ...
    second - before a line of Sabline is read. A pool pays that once
    ...
    * CLOSING KILLS EVERY WORKER, including one still running a
    ...
    is everywhere else in Sabline, and the same things sit outside it -
    ...
    def _worker_temp(self) -> str | None:
        ...
        if not self.confine:
            return None
        try:
            return tempfile.mkdtemp(prefix="sabline-worker-")
        except OSError:
            return None

    def _worker_options(self) -> dict[str, Any]:
        """What else a worker is started with: nothing, for a pool; sabline
