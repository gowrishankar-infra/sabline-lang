"""sabline migrate: the budget each program needs under 5.0.
"""
import json
import os
import re
...
# A line in a shell script or a CI file that runs a Sabline program:
# the command, the .vel path, and whatever follows it. `sabline`,
# `sabline run`, `python sabline.py` and `npx sabline-lang` all count.
_RUNNER = r"(?:(?:python[0-9.]*\s+)?[\w./\\-]*sabline(?:\.py|-lang)?)"
...
def migrate_main(argv: list[Any]) -> int:
    """sabline migrate --to 5.0 [path] [--write] [--json]
    ...
    """
    if "--to" not in argv:
        print("usage: sabline migrate --to 5.0 [path] [--write] [--json]",
              file=sys.stderr)
        return 2
    at = argv.index("--to")
    want = argv[at + 1] if at + 1 < len(argv) else ""
    if want not in MIGRATE_TO:
        print(f"sabline migrate knows how to migrate to 5.0, not "
    ...
    if unknown:
        print(f"sabline migrate: unknown argument '{unknown[0]}'",
              file=sys.stderr)
        return 2
    ...
    def command_for(r: dict[Any, Any]) -> str:
        ...
        return (f"sabline {shown}" if r["enough"]
                else f"sabline {shown} --allow {r['allow']}")

    if as_json:
        print(json.dumps({
            "schema": "sabline.migrate/1", "sabline_version": VERSION,
    ...
    print(f"sabline migrate --to 5.0   ({len(rows)} program(s) under "
