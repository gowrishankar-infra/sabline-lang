# Sabline for language models

Paste this whole file into a model before asking it to write Sabline.
It is written to be read by a model, not a person: complete, compact,
and with the failure modes named. `sabline card` prints it.

## This is a real language you can run

`pip install sabline-lang`, then `sabline program.vel`. There is a
browser playground at
<https://sabline.dev/playground.html> and
the compiler is one Python file at
<https://github.com/gowrishankar-infra/sabline-lang>. Write code
...
`sabline program.vel`, which grants `io` - print, read a line, read
...
Python has to be run with `--allow` naming it: `sabline program.vel
--allow io,fs:read:./data`. Check without running:
`sabline check program.vel`.
...
2. **Failure cannot be ignored.** These can fail: `to_int`,
   ...
   `json_int`, `json_float`, `json_len`, `hex_decode`, `base64_decode`,
   `tool`, `tool_secret`.
...
azure, github, k8s, aws  (8.5; all uses net, declassify, CAN FAIL; none
calls Python - prefer them to py() for these services. The token is a
Secret of Text: pass env("NAME", "") straight in, never declassify it
yourself. Items of a list come back as JSON text: read with json_get.)
  azure.resource_groups(token, subscription) -> List of Text
  azure.list(token, path, most_pages) / azure.read(token, path)
  azure.put_resource / patch_resource(token, path, body_json)
  azure.delete_resource(token, path)    grant net:management.azure.com:443
  github.issues / pulls(token, owner, repo, "open") -> List of Text
  github.repo / latest_release(token, owner, repo) -> Text
  github.releases / repos_of / check_runs(token, owner, repo, ref)
  github.file_text(token, owner, repo, path, ref) -> Text
  github.create_issue(token, owner, repo, title, body)
  github.create_comment(token, owner, repo, number, body)
                                        grant net:api.github.com:443
  k8s.cluster(server_url, token) / k8s.in_cluster() (uses fs) -> K8sCluster
  k8s.pods / services / deployments / configmaps / events(c, namespace)
  k8s.nodes(c) / namespaces(c) / pod(c, ns, name) / read(c, path)
  k8s.watch_once(c, path, resource_version, seconds) -> List of Text
  k8s.write_create / write_patch / write_delete / write_scale change the
    cluster; everything else only reads     grant net:<your API server>
  aws.credentials_from_env(region) (uses env, declassify) -> AwsCredentials
  aws.s3_list_buckets(creds) / s3_list_objects(creds, bucket, prefix, pages)
  aws.s3_get_object / s3_put_object / s3_delete_object(creds, bucket, key..)
  aws.sts_caller_identity(creds) -> Text     (aws also uses clock)
                                        grant net:s3.REGION.amazonaws.com:443

rest  (8.5; import "rest.vel" as rest; uses net, CAN FAIL)
  call_json(method, url, body_json, extra_headers_map) -> RestAnswer
  call_retrying(method, url, body, headers_json, attempts) -> RestAnswer
    (.status .body; at most ten attempts, on 429/502/503/504 or no answer)
  header_map(map) -> Text   items(body, path) -> List of Text   CAN FAIL
  succeeded(answer) -> Bool
...
sha256(t) -> 64 hex digits     url_encode(t)      (8.5, all pure, UTF-8)
hex_encode(t)  base64_encode(t)  hex_decode(t) CAN FAIL  base64_decode(t)
                                                          CAN FAIL
hmac_sha256(key, message) uses declassify   key MUST be a Secret of Text
hmac_sha256_chain(key, [m1, m2, ...]) uses declassify   HMAC again and
  again, each result the next key (AWS SigV4). The result is a plain
  Text; the message may not hold a secret. The audit lists the call as a
  declassification with the reason "hmac signature".

tool(name, arguments_json) CAN FAIL uses tool     a tool the host offers
tool_secret(name, arguments_json) CAN FAIL uses tool -> Secret of Text
  arguments: json_of({"query": text}). Only under sabline run --tools
  manifest.json; the budget grants tool:NAME and may hold an argument to
  a pattern (tool:send_email:to=*@corp.com). Outside the grant: E321.
...
declassify(secret, "why") uses declassify - the reason must be
  written here, not built; it is what sabline audit reports
...
```
sabline program.vel                      run it; since 5.0 it gets io -
                                         print, read_line, args - and
                                         every other effect is refused
                                         (E310), whatever the source says
sabline program.vel --allow io,fs:read:./data
                                         grant exactly this and no more
sabline program.vel --allow all          every effect; one line to stderr
sabline program.vel --allow all --deny net,ffi
                                         everything but these
sabline program.vel --allow io,ffi:math,json
                                         ffi for THOSE modules only (E311
                                         for any other); plain ffi grants
                                         every module
sabline check program.vel [--json]       every problem, as data
sabline check program.vel --strict       refuse ANY promise left to
                                         runtime; without it an
                                         unprovable promise degrades to
                                         a runtime check
sabline audit program.vel                what it touches, before running
sabline proofs program.vel --detail      which promises proved, one by one
sabline explain program.vel              functions, effects, proof status
```
...
declassify                  letting a Secret out, and signing with one
tool                        any tool the host offers (sabline run --tools)
tool:search                 that tool       tool:search@20   at most 20 calls
tool:send_email:to=*@corp.com   that tool, its `to` held to the pattern
```

Examples: `--allow io,fs:read:./data,net:api.example.com:443@20`;
`sabline.run(source, allow={"io", "env", "fs:write:./out"})`.
...
| E320 | a tool call with no tool to reach: no `--tools`, or not offered |
| E321 | a tool, or an argument, outside the `tool:` grants |
| E322 | a tool count or the manifest's ceiling was reached |
| E323 | arguments the tool's schema does not take |

A budget with no count is a budget on what, not on how much.

## Errors, and what to do about them

Every error has a stable code. `sabline check program.vel --json` emits
...
- Would `sabline check` pass? If unsure, prefer fewer contracts and
