"""sabline deps-diff: what one dependency's newer version gained.
"""
import json
import os
import re
...
#
#     `sabline deps-diff <package> <old> <new>` reads both versions of a
...
#     as unknown.
#
#     `sabline deps-diff --against REF` does the same for every dependency
#     the lockfiles changed since REF upgrade, and `--comment` puts the
#     answer in one pull-request comment that later runs edit.
# ---------------------------------------------------------------------------


DEPS_DIFF_SCHEMA = "sabline.deps-diff/1"
DEPS_LOCKFILES_SCHEMA = "sabline.deps-diff-lockfiles/1"
DEPS_COMMENT_MARKER = "<!-- sabline-deps-diff-comment -->"
...
def _deps_get(url: str, limit: int = _DEPS_ARTIFACT_LIMIT,
    ...
    headers = {"User-Agent": f"sabline/{VERSION} (deps-diff)"}
    if accept:
...
def _code_counts(files: dict[Any, Any]) -> dict[Any, Any]:
    ...
    not Sabline. Type declarations (.d.ts) are not code."""
    out: dict[Any, Any] = {}
    for p in files:
        low = p.lower()
...
def _read_pypi(name: str, version: str) -> dict[str, Any]:
    import urllib.parse
    base = naming.env("SABLINE_PYPI_URL", "https://pypi.org").rstrip("/")
...
def _read_npm(name: str, version: str) -> dict[str, Any]:
    ...
    import hashlib
    import urllib.parse
    base = naming.env("SABLINE_NPM_REGISTRY",
...
def _deps_sabline(a: dict[Any, Any], b: dict[Any, Any], scratch: str) -> dict[str, Any] | None:
    ...
    check = capabilities_compare(capabilities_document(scan_a, ""), scan_b,
                                 a["version"])
    # the check's own findings, less the edit to sabline.capabilities that
...
def _deps_result(package: str, kind: str, where: str, a: dict[Any, Any], b: dict[Any, Any],
    ...
    def side(v: Any, code: Any) -> dict[str, Any]:
        return {"version": v["version"], "read": v["read"],
                ...
                "files": len(v["files"]),
                "sabline_files": sum(1 for p in v["files"]
                                     if p.endswith(".vel")),
                "other_code": code, "notes": v["notes"]}

    return {"schema": DEPS_DIFF_SCHEMA, "sabline_version": VERSION,
            "package": package, "source": kind, "name": where,
            "old": side(a, code_a), "new": side(b, code_b),
            "capability": capability, "sabline": vel,
...
def deps_diff(package: str, old: str, new: str) -> dict[Any, Any]:
    ...
    a sabline.deps-diff/1 document. DepsError when either version cannot
    be read."""
    import shutil
    import tempfile
    ...
    scratch = tempfile.mkdtemp(prefix="sabline-deps-")
    try:
        if kind == "pypi":
...
def deps_diff_lines(result: dict[Any, Any]) -> list[Any]:
    """sabline deps-diff's report, as text."""
    old, new = result["old"]["version"], result["new"]["version"]
    L = [f"sabline deps-diff: {result['package']} {old} -> {new}"]
    ...
    if d is None:
        L.append("  declared dependencies: none read - neither version has "
                 "a manifest that declares them (a Sabline library names "
...
def _package_base_uri(result: dict[Any, Any]) -> str:
    ...
    return "urn:sabline-deps-diff:" + re.sub(r"[^A-Za-z0-9._@-]+", "-",
                                             result["package"]) + "/"


def sarif_deps_diff(result: dict[Any, Any]) -> dict[Any, Any]:
    """`sabline deps-diff PACKAGE OLD NEW --sarif`: each finding at the
...
def _parse_lockfile(fmt: str, text: Any) -> tuple[Any, ...]:
    ...
    if fmt == "npm-lock":
        ...
        npm_host = urllib.parse.urlsplit(naming.env(
            "SABLINE_NPM_REGISTRY", "https://registry.npmjs.org")).netloc.lower()
        ...
        try:
            ...
            return out, ["it is not JSON"]
        pypi = os.environ.get("SABLINE_PYPI_URL", "https://pypi.org").rstrip(
            "/")
...
def _vendored_diff(name: str, old_entry: dict[Any, Any], new_entry: dict[Any, Any],
    ...
    """deps-diff of a library sabline.lock vendors, between the file the
    ...
    scratch = tempfile.mkdtemp(prefix="sabline-deps-")
    try:
        result = _deps_result(f"sabline.lock:{name}", "vendored", file, a, b,
                              scratch)
    finally:
        shutil.rmtree(scratch, ignore_errors=True)
...
def deps_review(against: str, root: str = ".",
    ...
    sabline.deps-diff-lockfiles/1 document. RuntimeError when git cannot
    answer."""
    try:
        commit = _git(["rev-parse", "--verify", "--quiet",
    ...
    for rel in sorted(p for p in changed if _lockfile_format(p)):
        ...
        if fmt == "not-read":
            entry["notes"].append("sabline deps-diff does not read this "
                ...
                if eco == "sabline":
                    ...
                    for e in (ob, nb):           # the lockfile is the pull
                        ...
                        if not f.endswith(".vel") or f.startswith("/") \
                            ...
                            raise DepsError(
                                f"sabline.lock names {f!r} for {name}, "
    ...
    return {"schema": DEPS_LOCKFILES_SCHEMA, "sabline_version": VERSION,
...
def deps_review_lines(review: dict[Any, Any]) -> list[Any]:
    L = [f"sabline deps-diff: lockfiles changed since {review['against']} "
         f"({review['commit'][:7]}): "
...
def deps_markdown(review: dict[Any, Any]) -> str:
    ...
    if not review["lockfiles"]:
        ...
        L.append("")
        L.append("<sub>Posted by the Sabline action (deps-diff); this "
    ...
    L.append("Lockfiles changed since the base `"
             ...
             + ".")
    L.append("")
    L.append("For a Sabline library this compares the capability surface "
    ...
    if unread:
        ...
        L.append("Changed and not read, because deps-diff does not read "
                 "their format: " + ", ".join(f"`{_md(x['file'])}`"
                                              for x in unread) + ".")
    L.append("")
    L.append("<sub>Posted by the Sabline action (deps-diff); this comment is "
...
def sarif_deps_review(review: dict[Any, Any]) -> dict[Any, Any]:
    """`sabline deps-diff --against REF --sarif`: each finding at the line
...
def _github(method: str, url: str, token: str, body: Any = None) -> tuple[Any, ...]:
    ...
    req = urllib.request.Request(url, data=data, method=method, headers={
        ...
        "User-Agent": f"sabline/{VERSION} (deps-diff)",
...
def deps_diff_main(argv: list[Any]) -> int:
    """sabline deps-diff <package> <old> <new> [--json | --sarif]
    sabline deps-diff --against REF [path] [--json | --sarif | --markdown]
                      [--comment --pr N] [--max N]
    sabline deps-diff --from FILE [...the same outputs]"""
    usage = ("usage: sabline deps-diff <package> <old-version> <new-version> "
             "[--json | --sarif]\n"
             "       sabline deps-diff --against REF [path] "
             "[--json | --sarif | --markdown] [--comment --pr N] [--max N]\n"
             "       sabline deps-diff --from FILE "
    ...
    if not lockfile_mode:
        try:
            ...
            print(f"sabline deps-diff: {e}", file=sys.stderr)
            return 2
        if "--json" in opts:
    ...
    if "--from" in opts:
        try:
            ...
            print(f"sabline deps-diff: cannot read {opts['--from']}: {e}",
                  file=sys.stderr)
            return 2
        if not naming.schema_matches(review_doc.get("schema"),
                                     DEPS_LOCKFILES_SCHEMA):
            print(f"sabline deps-diff: {opts['--from']} is not a "
    ...
    if "--comment" in opts:
        try:
            ...
            print(f"sabline deps-diff: {e}", file=sys.stderr)
            return 2
        print(f"sabline deps-diff: {said}", file=sys.stderr)
