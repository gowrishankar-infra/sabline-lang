"""sabline replay: a run made again from its receipt, and every difference
named (8.3).
"""
import hashlib
import json
...
#
#     `sabline replay RECEIPT` reads each subject the receipt names from disk,
...
#     py, py_int, py_float and py_json. `sabline program.vel
#     --record-responses FILE` records what each such call gave back, and
#     `sabline replay --responses FILE` gives those back in the same order in
...
REPLAY_SCHEMA = "sabline.replay/1"            # provisional
RESPONSES_SCHEMA = "sabline.responses/1"      # provisional
...
def load_responses(path: str) -> ResponseReplay:
    ...
    if not (isinstance(doc, dict)
            and naming.schema_matches(doc.get("schema"), RESPONSES_SCHEMA)
...
def replay(receipt_path: str, *, root: str = ".", max_allow: str =
    ...
    """The sabline.replay/1 report, with its exit status in 'exit'."""
    ...
    home = tempfile.mkdtemp(prefix="sabline-replay-")
    try:
...
def _replay(report: dict[str, Any], receipt: dict[str, Any],
    ...
    if eval_profile:
        work = os.path.join(home, ".sabline-tmp")
...
def replay_main(argv: list[Any], program_words: list[str] | None = None) -> int:
    """sabline replay RECEIPT [--root DIR] [--max-allow G] [--stdin FILE]
    [--expect-output FILE] [--responses FILE] [--json] [-- ARGS...]"""
    usage = ("usage: sabline replay <receipt> [--root DIR] [--max-allow G] "
    ...
    # its first statement as `sabline program.vel` is (8.4). Its copy of the
    ...
    if report["refused"]:
        print(log_line(f"sabline replay: refused: {report['refused']}"),
              file=sys.stderr)
        return int(report["exit"])
