"""sabline eject: a directory that runs with nothing from this project.
"""
import json
import os
import sys
...
#
#     `sabline eject program.vel` writes a directory that runs, and builds
...
#     program says. The proofs are a record of what this Sabline proved at
#     eject time, which nothing trusts when the program runs, and which
#     `main.py --prove` runs again.
# ---------------------------------------------------------------------------


EJECT_SCHEMA = "sabline.eject/1"

_EJECT_LAUNCHER = r'''#!/usr/bin/env python3
"""@NAME@: a Sabline program, ejected by Sabline @VERSION@ on @DATE@.
...
def writes_here(sabline):
    ...
    run uses - or None."""
    budget = sabline.Budget.parse(BUDGET or "''")
...
def writes_where_python_imports(sabline):
...
def main(argv):
    ...
    if os.path.normcase(os.path.dirname(os.path.dirname(
            os.path.abspath(sabline.__file__)))) \
            != os.path.normcase(os.path.abspath(runtime)):
        print("refused: sabline was imported from " + sabline.__file__
              + ", not from this directory's runtime/", file=sys.stderr)
        return 2
    reach = writes_here(sabline)
    if reach:
        ...
        return 2
    reach = writes_where_python_imports(sabline)
    if reach:
        print("refused: the budget " + BUDGET + " lets the program write "
    ...
    if prove:
        sys.argv = [sabline.__file__, "check", entry]
        if getattr(sys, "frozen", False):
            sys.argv.append("--no-check-ceiling")
        return sabline.main()
    sys.argv = [sabline.__file__, entry, "--allow", BUDGET or "''"] + rest
    return sabline.main()
...
_EJECT_README = """# @NAME@, ejected from Sabline

This directory is `@ENTRY@` and everything it needs to run, taken out of
Sabline @VERSION@ on @DATE@ by `sabline eject`. Nothing in it fetches
anything, and it does not need Sabline installed.
...
| `runtime/sabline/` | the Sabline @VERSION@ compiler and runtime, unchanged |
| `runtime/stdlib/` | the standard library files the program imports |
| `program/` | the program, and the libraries it imports |
| `proofs.json` | what Sabline @VERSION@ proved when the program was ejected |
...
| `LICENSE` | Sabline's licence, which covers `runtime/` |

## What holds once ejected

- **The budget is enforced.** The program runs under `@BUDGET@` whatever
  its source declares, checked by `runtime/sabline/` at the moment each
  effect is attempted - the check `sabline program.vel --allow @BUDGET@`
...
- **The proofs are a record, not a promise.** `proofs.json` says what
  Sabline @VERSION@ proved when the program was ejected (@PROVER@). Nothing
...
- **No fix arrives.** `runtime/sabline/` is Sabline @VERSION@ and stays
  that. A later Sabline that closes a hole does not reach this directory;
...
- **What Sabline does not defend against**, it does not defend against
  here either (THREAT_MODEL.md in the Sabline repository): a granted `ffi`
...
def _sabline_license() -> str:
    ...
    try:
        ...
        dist = metadata.distribution("sabline-lang")
        for f in dist.files or []:
            if os.path.basename(str(f)).upper().startswith("LICENSE"):
                return f.read_text(encoding="utf-8")
    except Exception:
        pass
    return ("Sabline is released under the MIT License:\n"
            f"{REPOSITORY}/blob/main/LICENSE\n")


def eject_main(argv: list[Any]) -> int:
    """sabline eject <program.vel> [-o DIR] [--allow GRANTS] [--force]"""
    import datetime
    import hashlib
    import shutil
    usage = ("usage: sabline eject <program.vel> [-o DIR] [--allow GRANTS] "
             "[--force]")
    ...
    if not os.path.isfile(entry):
        print(f"sabline eject: no such file: {entry}", file=sys.stderr)
        return 2
    with open(entry, encoding="utf-8") as fh:
        source = fh.read()
    report = _audit_here(source, path=entry)
    if not report.ok:
        print(f"sabline eject: {entry} does not compile, so it was not "
    ...
    try:
        ...
        print("sabline eject: the program's files are on different drives, "
    ...
    if os.path.exists(out):
        ...
        try:
            with open(os.path.join(out, "proofs.json"),
                ...
                ours = naming.schema_matches(
                    json.load(fh).get("schema"), EJECT_SCHEMA)
        except (OSError, ValueError, AttributeError):
            ours = False
        if not (os.path.isdir(out) and not os.listdir(out)):
            if not (force and ours):
                print(f"sabline eject: {out} already exists"
                      + ("; --force replaces it" if ours else
                         " and was not made by sabline eject; choose "
    ...
    for name in sorted(os.listdir(_PACKAGE_DIR)):
        if name.endswith(".py"):
            placed[f"runtime/sabline/{name}"] = os.path.join(
                _PACKAGE_DIR, name)
    ...
    try:
        ...
        print(f"sabline eject: the copy does not load: [{e.code}] "
    ...
    if outside:
        shutil.rmtree(out, ignore_errors=True)
        print(f"sabline eject: the program imports {outside[0]} by a path "
    ...
    record = {"schema": EJECT_SCHEMA, "sabline_version": VERSION,
    ...
    pins = [f"# Pinned by sabline eject (Sabline {VERSION}, {date}) to what "
