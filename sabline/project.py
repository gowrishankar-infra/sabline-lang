"""A project's files: vendored libraries and sabline.lock, build, new, doctor.
"""
import json
import os
import sys
...
STARTER = """// Welcome to Sabline - the language where you can trust code you
// didn't write. Run me with:   sabline main.vel
...
MANIFEST = "sabline.toml"
LOCKFILE = "sabline.lock"
LOCK_SCHEMA = "sabline.lock/1"


def _manifest_read() -> list[Any]:
    """Every dependency, as (name, source, sha256)."""
    here = naming.existing(".", MANIFEST)
    if here is None:
        return []
    import re as _re
...
def _manifest_write(deps: list[Any]) -> None:
    with open(MANIFEST, "w", encoding="utf-8") as f:
        f.write("# Sabline dependencies. Every library is vendored into\n"
                "# lib/ and recorded here with the exact bytes it had, so\n"
                "# 'sabline deps --verify' can tell you if anything "
                "changed.\n\n")
...
#      which Sabline put them there - so a checkout on another machine
...
def verify_libraries() -> int:
    ...
    Against sabline.lock when there is one - which also knows which
    Sabline added each library - and against sabline.toml when there is
    ...
    for name, source, digest, path, added_by in rows:
        if not os.path.exists(path):
            print(f"  MISSING  {name} - {path} is not there "
                  f"(re-add it: sabline add {source})")
            bad += 1
            continue
        now = _digest_of(path)
        if now != digest:
            ...
            note = f"  (added by Sabline {added_by})" if added_by != "?" \
                else ""
        ...
        for name, source, _digest in sorted(deps):
            if name not in locked:
                print(f"  UNLOCKED {name} is in {MANIFEST} and not in "
                      f"{LOCKFILE} (re-add it: sabline add {source})")
                bad += 1
...
def packages(argv: list[Any]) -> int:
    ...
    if cmd == "deps":
        ...
        for name, source, digest in deps:
            ...
            print(f"  [{mark}] {name}\n      from {source}"
                  f"\n      {digest[:16]}..."
                  + (f"   added by Sabline {added}" if added else ""))
        if not lock:
            print(f"\nthere is no {LOCKFILE} yet - sabline add writes "
                  f"one, and 'sabline deps --verify' reads it")
        else:
            print(f"\n{LOCKFILE} records all of it; check it with: "
                  f"sabline deps --verify")
        return 0

    if len(argv) < 2:                     # add
        print("usage: sabline add <url or path> [as <name>] [--force]",
              file=sys.stderr)
        return 1
    force = "--force" in argv
    ...
    if source.startswith("http://") or source.startswith("https://"):
        try:
            ...
            print(f"could not fetch {source}: it {e.why} ({e.target}); "
                  f"'sabline add' refuses that redirect (8.0)",
                  file=sys.stderr)
            return 1
...
def build_program(argv: list[Any]) -> int:
    """Turn a Sabline program into one file anyone can run."""
    if not argv or argv[0].startswith("-"):
        print("usage: sabline build program.vel [-o name]",
              file=sys.stderr)
        return 1
    entry = argv[0]
    ...
    work = tempfile.mkdtemp(prefix="sabline-build-")
    launcher = os.path.join(work, f"{out_name}.py")
    with open(launcher, "w", encoding="utf-8") as f:
        f.write("# Generated by sabline build - a Sabline program,\n"
                ...
                "import sabline\n"
                "def main():\n"
                "    here = tempfile.mkdtemp(prefix='sabline-run-')\n"
    ...
    if "--for-everyone" in argv:
        ...
        with open(wf, "w", encoding="utf-8") as f:
            f.write(f"""# Built by sabline build --for-everyone.
  ...
  build:
    ...
    steps:
      ...
      - uses: actions/setup-python@v6
        with:
          python-version: "3.12"
      - run: pip install "sabline-lang[full]" pyinstaller
      - run: sabline build {entry} -o {out_name}
      - shell: bash
        run: |
...
""")
    ...
    print("that file is the whole program - no Python, no Sabline, "
          "nothing to install")
    return 0
...
def doctor() -> int:
    ...
    if (pv.major, pv.minor) >= (3, 10):
        ...
        healthy = False
        lines.append(f"{BAD} python {pv.major}.{pv.minor} - Sabline "
                     f"needs 3.10+ (install from python.org)")
    here = _PACKAGE_DIR
    lines.append(f"{OK} sabline {VERSION}  ({here})")
    try:
        import z3  # noqa: F401
    ...
    if os.path.exists(std):
        ...
        lines.append(f"{BAD} standard library missing - reinstall: "
                     f"pip install --force-reinstall sabline-lang")
    try:
        ...
        if errs:
            raise SablineError("E999", "self-test failed", 1)
    ...
    print(f"sabline doctor - {VERSION}")
    print("-" * 60)
    for ln in lines:
        print(ln)
    print("-" * 60)
