...
MUTABLE_GLOBALS (sabline/pool.py) lists the names a pool worker puts back
between programs.
"""
...
PROGRAM_ARGS: list[str] = []    # filled by the CLI: sabline prog.vel a b c
...
# what a receipt's `grants_used` is made from. TOOL_SESSION is the session `sabline
# run --tools` opened (sabline/tools.py), or None: then there is no tool to
...
# are recorded as the run's parameters (sabline.invocation/1 run_params on
...
# the worker of `sabline eval`, which makes it in a directory of its own,
...
# or handed back in order by `sabline replay --responses FILE`, in place of
...
# sabline/confine.py). True unless --no-confine, Pool(confine=False) or
