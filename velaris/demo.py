"""`velaris demo` (8.5): one refusal and one run inside a budget, with their
...
DEMO_ENV = ("# written by `velaris demo`; nothing here is real\n"
...
def demo_main(argv: list[Any]) -> int:
    ...
    if any(a != "--keep" for a in argv):
        print("usage: velaris demo [--keep]\n  it takes no program, path, "
              "address or budget: it writes what it runs", file=sys.stderr)
        return 2
    keep = "--keep" in argv
    here = os.path.realpath(tempfile.mkdtemp(prefix="velaris-demo-"))
    try:
...
def _demo(here: str, keep: bool) -> int:
    ...
    out = [f"velaris demo: one task, run twice, in {here}", ""]

    out.append("1. A script that reads ./.env and posts it to a webhook. No "
               "budget is given, so it gets io:")
    out.append("")
    out.append(f"   $ velaris agent_script.vel --receipt {REFUSED_RECEIPT}")
    ...
    out.append("")
    out.append(f"   $ velaris inside_budget.vel --allow {budget} --receipt "
               f"{ALLOWED_RECEIPT}")
    ...
    out.append("")
    if keep:
        out.append(f"The files are in {here}; `velaris receipt show "
                   f"{REFUSED_RECEIPT}` there renders a receipt as a page.")
    else:
        out.append("Nothing is left behind; `velaris demo --keep` leaves "
    ...
    held = (status == 1 and refusal.get("code") == "E310"
            ...
            .get("outcome") == "ok")
    if not held:
        print("velaris demo: this did not go as it should have - the first "
