"""Reading the in-toto Statements Sabline writes - attestations and receipts -
and holding one to the bytes it names: `sabline verify` (8.3).
"""
import hashlib
import json
import os
...
_NAMED_UNTIL = {"https://velaris-lang.dev": "8.5 (before the rename)",
                "https://gowrishankar-infra.github.io/velaris-lang": "8.2.1"}


def _named_until(predicate_type: object) -> str:
    for site, release in _NAMED_UNTIL.items():
...
#
#     `sabline verify FILE` reads a Statement Sabline wrote, as it is, in a
#     DSSE envelope, or in a Sigstore bundle, and says three things: whether
#     its predicate type is one Sabline defines (capability/v1 or receipt/v1,
#     at sabline.dev or at either address the project used before it -
#     the GitHub Pages one until 8.3, velaris-lang.dev until the rename in
#     8.6, and nothing else), whether its predicate has the shape that type requires, and
...
VERIFY_SCHEMA = "sabline.verify/1"          # provisional (STABILITY.md)
...
def read_statements(path: str) -> list[tuple[dict[str, Any], str, Any]]:
    ...
    as `sabline attest DIR` writes - each as (Statement, carrier, the
...
def statement_problems(statement: dict[str, Any]) -> tuple[str | None, list[str]]:
    ...
    is wrong with it). A predicate type Sabline does not define is a
    ...
    if kind is None:
        problems.append(
            ...
            f"one Sabline defines: {CAPABILITY_PREDICATE_TYPE} and "
            f"{RECEIPT_PREDICATE_TYPE}, or either at an address this "
            f"project used before ("
            + ", ".join(EARLIER_SITES) + ")")
    ...
    if not isinstance(predicate, dict):
        ...
        if not (isinstance(audit, dict)
                and naming.schema_matches(audit.get("schema"),
                                          "sabline.audit/1")):
            problems.append("its predicate holds no sabline.audit/1 document")
...
def receipt_shape_problems(predicate: dict[str, Any]) -> list[str]:
    """What a sabline.receipt/1 document lacks that section 8.7 requires,
    and what it holds of the wrong type - a count that is not a whole
    number of at least zero among them."""
    out: list[str] = []
    if not naming.schema_matches(predicate.get("schema"), RECEIPT_SCHEMA):
...
def subject_file(name: str, root: str) -> tuple[str | None, str]:
    ...
    is read inside `root`, or inside this Sabline's standard library for
...
def verify_file(path: str, *, root: str, identity: str | None = None,
    ...
    """The sabline.verify/1 report of one file, and its exit status: 0 when
...
def verify_main(argv: list[Any]) -> int:
    """sabline verify STATEMENT [--root DIR] [--identity ID] [--issuer URL]
    [--skip-signature] [--json]"""
    usage = ("usage: sabline verify <statement> [--root DIR] [--identity ID] "
    ...
    while i < len(argv):
        ...
        if a in ("--root", "--identity", "--issuer"):
            if i + 1 >= len(argv):
                print(f"sabline verify: {a} needs a value", file=sys.stderr)
                return 2
            if a == "--root":
                root = argv[i + 1]
            elif a == "--identity":
    ...
    for s in report["statements"]:
        ...
        say(f"type:      {s['predicateType']!s}"
              + (f" ({kind}/v1" + (f", as Sabline named it until "
                                   f"{_named_until(s['predicateType'])}"
                                   if s["earlier_spelling"] else "") + ")"
                 if kind else " (REFUSED: not a type Sabline defines)"))
        sig = s["signature"]
        if not sig["carried"]:
        ...
        for p in s["not_checked"]:
            if not any(p == f"{r['name']}: {r['why']}" for r in s["subjects"]):
                say(f"  NOT CHECKED  {p}")
        say({"verified": "verified: a type Sabline defines, and every "
