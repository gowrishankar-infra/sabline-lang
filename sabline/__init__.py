"""Sabline - the language where you can trust code you did not write.
...
lists, which this file binds once all are loaded. `import sabline`
gives every name sabline.py gave while it was one file (until 8.2),
and the run state (sabline/state.py) is read where it lives.
"""
import sys as _sys
...
from .errors import (  # noqa: E402
    ...
    SablineError as SablineError,
    # the name this class had until 8.6 renamed the project; the same
    # class, so `except velaris.VelarisError` still catches (STABILITY.md)
...
from .upgrades import (  # noqa: E402
    ...
    _deps_sabline as _deps_sabline,
...
from .eject import (  # noqa: E402
    ...
    _sabline_license as _sabline_license,
...
def __getattr__(name: str) -> _typing.Any:
    """The run state, read where it lives: sabline.EFFECT_BUDGET is
    sabline.state.EFFECT_BUDGET as it is at this moment."""
...
class _Package(_types.ModuleType):
    ...
    they are read. Until 8.1.1 `sabline.IMPORT_ROOT = root` set the one
    ...
    8.2 adversarial pass). A write to a name sabline.state holds now reaches
    sabline.state, as a read does."""
