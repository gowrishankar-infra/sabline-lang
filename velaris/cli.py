...
Usage:
  ...
  velaris check f.vel --json               the problems as JSON
...
# advisory-proof-cache-2.md); 8.2 keeps nothing. `--no-cache` and `velaris
# clean` are accepted and do nothing but say so, until 9.0.
NO_CACHE_NOTICE = ("velaris: --no-cache is deprecated and does nothing: "
...
def usage_lines() -> dict[Any, Any]:
    """{command: its lines in the usage list above} - the list `velaris`
    ...
    for line in block.splitlines():
        ...
        elif not line.startswith("    "):     # `  python velaris.py ...`
            current = None
...
def print_usage(command: str | None = None) -> int:
    """`velaris --help`, or `velaris <command> --help`: the usage, and 0."""
    if command is None:
        block = (__doc__ or "").split("\nUsage:\n", 1)[-1].split("\n\n", 1)[0]
...
def _vel_files_under(target: str) -> list[Any]:
    ...
    --sarif` left out every path holding the text ".velaris" - the old
    proof cache's folder, and with it a folder such as cfg.velaris.d/ - so
...
def main() -> int:
    ...
    if reconfigure is not None:
        try:
            ...
            pass
    # `--` ends Velaris's own flags on a run (8.2): a file, `run` or `trace`.
    ...
    # occurrence won, so `velaris a.vel -- --allow all` granted every effect
    ...
    if argv[:1] == ["audit"]:
        ...
        grants = ",".join(outside)
        if grants:
            print(f"  velaris {target} --allow {grants}")
    ...
    if argv[:1] == ["clean"]:
        ...
        print("velaris clean: deprecated, and does nothing - Velaris keeps "
              "no proofs from 8.2, so there is nothing to clean. A "
              "'velaris' folder an earlier version left in your user cache "
