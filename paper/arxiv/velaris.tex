...
escapes the function-level rule. On a benchmark of 68 programs, 60 with
one defect and 8 correct, each written in Velaris, in JavaScript for
Deno and in Python, Velaris caught 58 of the 60 defects, 46 of them
before running; Deno caught 36 and Python 28; none of the three flagged
...
same behaviour: in Velaris, in JavaScript for Deno, and in Python. Sixty
contain one deliberate defect, on one line marked in all three sources;
eight are correct controls. They fall in thirteen categories: a file
write hidden in a helper, a network call hidden in a helper, division by
a value that can be zero, a read past the end of a list, integer
overflow, an ignored failure, an infinite loop, runaway memory, reaching
a dangerous module, correct programs that must not be flagged, a grant
narrower than the effect, indirect authority - a caller that does not
change while a dependency's declared budget widens between two versions
- and a TrapDoor, a program whose stated purpose and behaviour differ.
One harness runs every program through every tool with the same rules: a
5-second timeout, a 256 MB memory cap, and for Velaris the narrowest
...
received by a local listener, a subprocess's sentinel printed. The
committed results were produced by Velaris 8.0.0, Deno 2.9.6 and Python
3.13.12 on Windows 11, and the 67 programs of the first twelve
categories kept every verdict and every line of evidence they had at
Velaris 7.1.1. Before that, ten consecutive runs at 7.1.0 had written
identical files, and the run at 7.1.1 changed only the version it
records. The 63 programs of the first eleven categories kept the verdict
each had under Velaris 4.1.0; Velaris 3.0.0, with the same Deno and
Python on the same platform, had produced that table before 4.1.0, every
verdict and every line of evidence.
...
\texttt{benchmark/results.json} at Velaris 8.2.0, written by Velaris
8.0.0.
...
categories and the thirteenth of a 68-program benchmark they caught 55
of 57 defects; the fourth's comparison, applied to two versions of a
dependency, caught the three defects of the twelfth; and no correct
program was flagged. The fourth is a property, not a rate: under a
required check, the declared surface does not widen without the check
failing, however the change is divided among commits. What it does not
do - attribute an effect to the right function across a rename, see into
a granted module, tell harmless text from a dangerous command - is
...
conclusion are from \texttt{benchmark/results.json} at Velaris 8.2.0,
which Velaris 8.0.0 wrote. Releases after 4.2.1 postdate the paper and
are not reflected in it: 4.3.0 added \texttt{Money\ of\ CUR}, an exact
decimal whose split is proven to add back up; 4.3.1 made a proof that
exhausts its time budget say so rather than fall silently back to a
runtime check; 4.4.0 added a reference platform service; and 5.0.0 made
\texttt{io} the budget a run gets when nobody writes one, where every
version this paper measured granted all seven effects - the related-work
paragraphs on WASI and on Boruna say what changed, and velaris-spec
0.6.0 restates its sections 4.4 and 4.6 to match; 6.0.0 and 7.0.0 added
\texttt{Secret\ of\ T}, a value a program cannot print, send or branch
on without declassifying it with a stated reason; 7.1.0 added
\texttt{velaris\ deps-diff} and the benchmark's twelfth category; 7.1.1
made 7.1.0 import on Python 3.10 and 3.11, and changed nothing the
benchmark measures; 7.1.2 and 8.1.1 closed two ways for a planted proof
cache to report a false promise as proven; 7.2.0 changed how releases
are made; 8.0.0 made four changes that break programs or commands 7.x
accepted, and added the benchmark's thirteenth category; 8.1.0 added
receipts of a run; and 8.2.0 split the compiler into one module per
stage and keeps no proof cache at all. Later patches corrected
...
\bottomrule\noalign{}
\endlastfoot
68 programs, 60 dangerous, 8 controls, thirteen categories; 46/12/2,
5/31/24, 0/28/32; 0 false positives; Velaris 8.0.0, Deno 2.9.6, Python
3.13.12, Windows 11; 5 s timeout, 256 MB cap; the misses \texttt{04c}
and \texttt{09c}; 55 of 57 in categories 1 to 11 and 13; category 12's
three upgrades and one control; category 13's program, its verdicts and
E318 & \texttt{benchmark/results.json} at v8.2.0 \\
three of each six programs written against the tools; 5 to 8 minutes for
a full run & \texttt{benchmark/README.md} at v8.2.0 \\
