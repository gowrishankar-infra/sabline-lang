...
The committed results were produced by Velaris 8.3.0, Deno 2.9.6 and
Python 3.13.13 on Windows 11, in ten consecutive runs that wrote
identical files, and the 68 programs of the first thirteen categories
kept every verdict they had at Velaris 8.2.0. Before that, the 67
programs of the first twelve categories kept every verdict and every
line of evidence they had at Velaris 7.1.1, and ten consecutive runs at
7.1.0 had written identical files. The 63 programs of the first eleven
categories kept the verdict each had under Velaris 4.1.0; Velaris 3.0.0,
with the same Deno and Python on the same platform, had produced that
table before 4.1.0, every verdict and every line of evidence.
...
\texttt{benchmark/results.json} at Velaris 8.3.0.
...
conclusion are from \texttt{benchmark/results.json} at Velaris 8.3.0.
Releases after 4.2.1 postdate the paper and are not reflected in it:
4.3.0 added \texttt{Money\ of\ CUR}, an exact decimal whose split is
proven to add back up; 4.3.1 made a proof that exhausts its time budget
say so rather than fall silently back to a runtime check; 4.4.0 added a
reference platform service; and 5.0.0 made \texttt{io} the budget a run
gets when nobody writes one, where every version this paper measured
granted all seven effects - the related-work paragraphs on WASI and on
Boruna say what changed, and velaris-spec 0.6.0 restates its sections
4.4 and 4.6 to match; 6.0.0 and 7.0.0 added \texttt{Secret\ of\ T}, a
value a program cannot print, send or branch on without declassifying it
with a stated reason; 7.1.0 added \texttt{velaris\ deps-diff} and the
...
one module per stage and keeps no proof cache at all; and 8.3.0 added an
evaluation profile, a comparison and a replay of receipts, and the
benchmark's fourteenth and fifteenth categories. Later patches corrected
...
8/34/24, 0/31/35; 0 false positives; Velaris 8.3.0, Deno 2.9.6, Python
...
\texttt{benchmark/README.md} at v8.3.0 \\
the 68 programs of categories 1 to 13 unchanged in verdict since 8.2.0;
ten identical runs at 8.3.0; categories 14 and 15 added in 8.3 &
\texttt{benchmark/results.json} at v8.2.0 and at v8.3.0, and
\texttt{CHANGELOG.md}, 8.3 entry \\
