...
\citep{miller2006robust}. A Sabline effect is a name in a signature, and
...
\citep{odersky2026tacit}. In Sabline capabilities are effect names plus
...
Sabline tracks nothing about data: a budget bounds which effects, paths,
...
so a module given nothing reaches nothing \citep{wasi}. Sabline's grants
...
Sabline refusal ends the run. Deno checks at the call; Sabline also
...
may have \citep{lucassen1988effects}. Sabline's \texttt{uses} is a fixed
...
analysis tools \citep{sarif2020}. sabline-lang writes its compile
...
describe. Later patches corrected documentation and packaging. What each
