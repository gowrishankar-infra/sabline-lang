"""Sabline from inside another program: check, audit, run, card.
"""
import os
import re
import sys
...
# ---------------------------------------------------------------------------
# 14. THE LIBRARY — Sabline from inside another program
#
...
#
#         import sabline
#         print(sabline.check(src).ok)
#         print(sabline.audit(src).effects)
#         print(sabline.run(src, allow={"io"}).output)
# ---------------------------------------------------------------------------


def _source_to_file(source: str, path: str | None) -> tuple[Any, ...]:
    """Sabline resolves imports against a file, so give the text one, in a
    ...
    if path is not None:
        return path, None
    home = tempfile.mkdtemp(prefix="sabline-source-")     # 0700 on POSIX
...
def _ffi_named(path: str, source: str | None) -> tuple[set[str], bool]:
    ...
    (4.0, sabline-spec Q4), without which a computed module name was
    invisible in sabline.audit/1."""
    try:
...
def _secrets_named(path: str, source: str | None) -> dict[Any, Any] | None:
    """sabline.audit/1's `secrets` (6.0): which builtins handed this
...
def _tools_named(path: str, source: str | None) -> dict[str, Any] | None:
    """sabline.audit/1's `tools` (8.5): the tools a program's calls name as
...
def check(source: str, *, path: str | None = None, prove: bool = True,
    ...
    `sabline check` has had since 8.0. Source crafted to stall the prover
    ...
    startup; sabline.Pool(...).check() keeps one alive.
...
def audit(source: str, *, path: str | None = None,
    ...
    The same answer `sabline audit` prints, as data, with a schema name
...
def _unfinished_audit(problem: "Problem") -> AuditResult:
    """The sabline.audit/1 document of an audit that was stopped: ok false,
    the problem that stopped it, and nothing determined."""
    return AuditResult(
        schema=AUDIT_SCHEMA, sabline_version=VERSION, ok=False,
        problems=[problem], effects=[], functions=[], proven_share=None,
        safe_command="sabline <file> --allow ''",
...
def _audit_confinement(safe_budget: str) -> dict[str, Any] | None:
    ...
    receipt, and what this machine offers is `sabline doctor`'s to say.
...
def _audit_here(source: str, *, path: str | None = None) -> AuditResult:
    ...
    try:
        ...
        # the audited file's functions can perform, by sabline-spec 9.4's
        ...
        return AuditResult(
            schema=AUDIT_SCHEMA, sabline_version=VERSION,
...
def run(source: str, *, path: str | None = None,
    ...
    or allow=set(sabline.ALL_EFFECTS).
    ...
    Calling this in a loop starts an interpreter every time. sabline.Pool
    ...
    program has been read and compiled (sabline/confine.py; the receipt's
...
#     behaviour Sabline had on Windows before 3.1 - rather than the run
...
class _WindowsMemoryJob:
    ...
    def __init__(self, max_memory_mb: int, one_process: bool = False) -> None:
        """`one_process` (8.3, sabline eval): the job is to hold the worker
    ...
    def hold_current(self) -> int | None:
        ...
        sabline eval's profile promises. Gives the count it held, or None
