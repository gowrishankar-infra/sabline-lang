"""`sabline skill verify DIR` (8.5): what a skill would need, before it runs.

A skill here is a directory: the programs it runs (`*.vel`, anywhere under
it), the tool manifest its host offers them (`tools.json`, a sabline.tools/1
...
cannot be read. `--json` writes sabline.skill-verify/1, which is provisional
(STABILITY.md).
"""
import json
import os
...
SKILL_VERIFY_SCHEMA = "sabline.skill-verify/1"
...
def skill_report(root: str, manifest_path: str | None = None
    ...
    """The sabline.skill-verify/1 document of the skill at `root`.
...
def skill_main(argv: list[Any]) -> int:
    """sabline skill verify DIR [--tools MANIFEST] [--json]"""
    usage = "usage: sabline skill verify <dir> [--tools MANIFEST] [--json]"
    if argv[:1] != ["verify"]:
    ...
    try:
        ...
        print(f"sabline skill verify: {e}", file=sys.stderr)
        return 2
    if as_json:
