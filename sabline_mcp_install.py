#!/usr/bin/env python3
"""Set up Sabline's MCP tools in whatever assistant this machine has.
...
adds a `sabline` server to each without disturbing anything else, and
says what it did.

    sabline mcp-install              set it up everywhere it finds
...
`sabline.mcpb` - double-click, no files edited. This is for everything
...
def entry() -> dict[str, Any]:
    ...
    return {"command": sys.executable, "args": ["-m", "sabline_mcp"]}
...
def install_into(path: Path, remove: bool = False) -> str:
    ...
    if path.exists():                      # never lose a working config
        shutil.copy2(path, path.with_suffix(path.suffix + ".sabline-backup"))
...
def main(argv: list[Any]) -> int:
    ...
    if not found:
        ...
        print("Sabline still works from the command line and as a library;")
    ...
    if listing:
        ...
        print()
        print('   "Use the sabline tools: read the card, write a small')
