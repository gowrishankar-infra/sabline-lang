"""sabline stats: counts over a directory of programs.
"""
import json
import os
import sys
...
#
#     `sabline stats --ffi <dir>` (8.2) counts the Python a directory of
...
#     sabline.stats-ffi/1, is provisional (STABILITY.md).
# ---------------------------------------------------------------------------


STATS_FFI_SCHEMA = "sabline.stats-ffi/1"
...
def stats_ffi(root: str, *, timeout: Any = CHECK_TIMEOUT_DEFAULT,
    ...
    is audited on one worker under the check ceiling, as `sabline audit`
    ...
    with Pool(size=1, timeout=timeout, max_memory_mb=max_memory_mb) as pool:
        for path in files:
            ...
            for m in named:
                ...
                entry["programs"] += 1
    return {"schema": STATS_FFI_SCHEMA, "sabline_version": VERSION,
...
def stats_ffi_lines(result: dict[Any, Any]) -> list[Any]:
    """What `sabline stats --ffi` prints."""
    counted = result["files"] - len(result["not_counted"])
    out = [f"sabline stats --ffi {result['root']}: {result['files']} "
...
def stats_main(argv: list[Any]) -> int:
    """sabline stats --ffi [dir] [--json]"""
    words = [a for a in argv if a not in ("--ffi", "--json")]
    if "--ffi" not in argv or len(words) > 1 \
            or any(w.startswith("-") for w in words):
        print("usage: sabline stats --ffi [dir] [--json]", file=sys.stderr)
        return 2
    root = words[0] if words else "."
    if not os.path.isdir(root):
        print(f"sabline stats: {root} is not a directory", file=sys.stderr)
        return 2
