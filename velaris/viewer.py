...
`velaris receipt show FILE` and `velaris audit FILE --html` write one HTML
...
def _grant_rows(predicate: dict[str, Any]) -> dict[str, list[tuple[str, Any]]]:
    ...
    host the program asked for (velaris-spec 8.7)."""
...
def audit_model(audit: dict[str, Any]) -> list[tuple[str, Any]]:
    ...
    out.append(("Written by", f"velaris-lang {audit.get('velaris_version')}, "
...
def render_html(title: str, lead: str, sections: list[tuple[str, Any]]) -> str:
    ...
    parts += [f'<p><a href="{_e(SITE)}/receipt/v1/">What a receipt is</a>, '
              f'and <a href="{_e(SITE)}/">Velaris</a>.</p>',
              "</main>", "</body>", "</html>"]
    return "\n".join(parts) + "\n"
...
def receipt_main(argv: list[Any]) -> int:
    """velaris receipt show RECEIPT [--text] [-o FILE]"""
    usage = "usage: velaris receipt show <receipt> [--text] [-o FILE]"
    if argv[:1] != ["show"]:
    ...
    while i < len(argv):
        ...
        if a == "--text":
            ...
            if i + 1 >= len(argv):
                print(f"velaris receipt show: {a} needs a file",
                      file=sys.stderr)
                return 2
            out_to = argv[i + 1]
            i += 1
    ...
    try:
        ...
        print(f"velaris receipt show: {e}", file=sys.stderr)
        return 2
    page = receipt_text(statement) if as_text else receipt_html(statement)
    return write_page(page, out_to, "velaris receipt show")
