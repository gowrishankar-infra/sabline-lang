"""velaris test --from-contracts: each function's requires, asked of the
...
#     the inputs a program happened to be given. `velaris test FILE
...
FROM_CONTRACTS_SCHEMA = "velaris.from-contracts/1"   # provisional
...
def _run_one(rt: dict[str, Any], fn: Any, args: list[Any], seconds: float,
    ...
    # `velaris test --from-contracts` rather than a report - and
...
def from_contracts(path: str, count: int = WITNESS_COUNT_DEFAULT,
    ...
    """The velaris.from-contracts/1 report of one file, with its exit status
    ...
    targets = [f for f in funcs if f.requires and not f.is_lambda
               ...
               == entry]
    home = tempfile.mkdtemp(prefix="velaris-witnesses-")
...
def from_contracts_main(argv: list[Any]) -> int:
    """velaris test FILE --from-contracts [--count N] [--witness-seconds S]
    [--json]"""
    usage = ("usage: velaris test program.vel --from-contracts [--count N] "
    ...
    while i < len(argv):
        ...
        if a in ("--count", "--witness-seconds"):
            try:
                ...
                print(f"velaris test: {a} needs a number: --count from 1 to "
    ...
    print(f"velaris test --from-contracts: {files[0]} (up to {count} "
