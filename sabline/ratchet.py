"""The capability ratchet (sabline capabilities), and sabline review.
"""
import json
import os
import re
...
#     sabline.capabilities, and `sabline capabilities check` fails a
...
#     `sabline review --against REF` reports the delta from a git ref to
...
#     same with and without z3. sabline-spec SPEC.md section 9 states
#     every rule used here.
# ---------------------------------------------------------------------------


CAPABILITIES_SCHEMA = "sabline.capabilities/1"
CAPABILITIES_FILE = "sabline.capabilities"
CAPABILITIES_CHECK_SCHEMA = "sabline.capabilities-check/1"
REVIEW_SCHEMA = "sabline.review/1"
...
# ---- grants as a baseline writes them (sabline-spec 9.2, 9.4) --------------
...
def _norm_path(p: str) -> str:
    """sabline-spec 9.4's N: `/` runs made one, `.` components removed,
...
def _path_covers(q: str, p: str) -> bool:
    """Does a grant for path q cover path p (sabline-spec 9.4)? A path
...
def _covers(b: tuple[Any, ...], c: tuple[Any, ...]) -> bool:
    """Does grant b cover grant c (both _grant_parts)? sabline-spec 9.4:
...
def _reduce_grants(grants: Any) -> list[Any]:
    ...
    (sabline-spec 9.2 rule 5). Of two grants that cover each other - two
...
# ---- what a program needs, from its text (sabline-spec 9.3) ----------------
...
def _text_constants(fn: Any) -> dict[Any, Any]:
    ...
    value built while running (sabline-spec 9.3)."""
    import dataclasses as _dc
    lets: dict[Any, Any] = {}
    order: list[Any] = []
...
def _needs(effects: Any, lits: dict[Any, Any], funcs: list[Any], own: list[Any]) -> dict[Any, Any]:
    """{grant: [sites]} - the grants sabline-spec 9.3 derives for a
...
def _program_capabilities(path: str, rel: str) -> dict[Any, Any]:
    """What one .vel file needs (sabline-spec 9.3): its grants, reduced;
    ...
    try:
        ...
        errors.append(SablineError(         # means: cannot be compared
...
def read_capabilities(path: str) -> dict[Any, Any]:
    """A sabline.capabilities/1 document, checked; ValueError naming the
    first thing wrong with it. sabline.capabilities/0, the spec's
    provisional form, was never written by Sabline and is refused."""
    try:
    ...
    if naming.schema_matches(doc.get("schema"), "sabline.capabilities/0"):
        raise ValueError(f"{path} is sabline.capabilities/0, the "
                         f"provisional form sabline-spec 0.2 described and "
                         f"no Sabline wrote; write a {CAPABILITIES_SCHEMA} "
                         f"one with: sabline capabilities init --force")
    if not naming.schema_matches(doc.get("schema"), CAPABILITIES_SCHEMA):
        raise ValueError(f"{path} is not a {CAPABILITIES_SCHEMA} document "
...
# ---- the ratchet: the tree against the baseline (sabline-spec 9.5) ----------
...
def capabilities_compare(baseline: dict[Any, Any], scan: dict[Any, Any],
    ...
    """The ratchet (sabline-spec 9.5): the tree in `scan` against the
    ...
    if written_by != VERSION:
        ...
        warnings.append(
            f"{baseline_file} was written by Sabline {written_by}, "
            ...
            f"in mind, then record the surface again with: sabline "
    ...
    return {"schema": CAPABILITIES_CHECK_SCHEMA, "sabline_version": VERSION,
            "root": root,
            "baseline": {"file": baseline_file,
                         "sabline_version": written_by,
...
def sarif_capabilities(result: dict[Any, Any]) -> dict[Any, Any]:
    """`sabline capabilities check --sarif`: each widening as an error at
...
def capabilities_main(argv: list[Any]) -> int:
    """sabline capabilities init [path] [--force]
    sabline capabilities check [path] [--json | --sarif] [--check-timeout S]
        [--check-memory-mb M] [--no-check-ceiling]"""
    usage = ("usage: sabline capabilities init [path] [--force]\n"
             "       sabline capabilities check [path] [--json | --sarif]\n"
    ...
    if not os.path.isdir(root):
        print(f"sabline capabilities: {root} is not a directory",
              file=sys.stderr)
        return 2
    target = (naming.existing(root, CAPABILITIES_FILE)
              or os.path.join(root, CAPABILITIES_FILE))
    ...
    if not os.path.exists(target):
        print(f"no {CAPABILITIES_FILE} in {root}: write one with sabline "
              f"capabilities init {root}", file=sys.stderr)
        return 2
    try:
        baseline = read_capabilities(target)
    except ValueError as e:
        print(f"sabline capabilities check: {e}", file=sys.stderr)
        return 2
    ...
    if result["widened"]:
        print(f"{len(result['findings'])} widening(s): the code needs more "
              ...
              f"    sabline capabilities init {root} --force")
    else:
...
def review(against: str, root: str = ".") -> dict[str, Any]:
    ...
    scratch = tempfile.mkdtemp(prefix="sabline-review-")
    try:
    ...
    declared = _declared_change(
        declared_before,
        naming.existing(root, CAPABILITIES_FILE)
        or os.path.join(root, CAPABILITIES_FILE))
    if declared["removed"]:
    ...
    return {"schema": REVIEW_SCHEMA, "sabline_version": VERSION,
            "declared": declared,
...
def _declared_change(before_text: Any, after_path: str) -> dict[Any, Any]:
    """How the declared surface - sabline.capabilities - changed between
...
def review_main(argv: list[Any]) -> int:
    """sabline review --against REF [path] [--json]"""
    usage = "usage: sabline review --against REF [path] [--json]"
    ref = None
    ...
    try:
        ...
        print(f"sabline review: {e}", file=sys.stderr)
        return 2
    if as_json:
    ...
    print(f"sabline review: {root} against {ref} ({got['commit'][:7]})")
