...
Why Sabline embeds in-process from Python and only Python, and what the process boundary costs from every other language, is on a page of its own: [docs/embedding-limit.md](docs/embedding-limit.md).
...
steps:
  ...
  - uses: gowrishankar-infra/sabline-lang@269c47aa1ab1b38efdfc13da828340d326fd98f7  # v8.7.0
    with:
      min-proven: "80"
      pr-comment: "true"
...
`sabline receipts diff` compares a receipt with the program's audit or with earlier runs of the same bytes, and `sabline replay` makes the run again from it: [docs/receipts.md](docs/receipts.md).
