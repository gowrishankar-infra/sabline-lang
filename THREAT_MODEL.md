...
|---|---|---|---|
| no `fs` granted | Landlock: no file read, written, made or removed but what the interpreter itself needs (below) | writes refused everywhere but the private temporary directory; reads refused under the home directory and /Volumes | low integrity level: no write to anything of the user's; reads not held |
